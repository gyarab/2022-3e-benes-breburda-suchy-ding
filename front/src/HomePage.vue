    ...
    <div class="flex flex-col w-1/2 items-center">
      <div class="w-full rounded-xl bg-[#1D1D2A] mt-4">
        <div class="flex h-12 items-center">
          ...
          <input v-model="search" @input="doSearch" placeholder="Search" class="block h-12 bg-transparent -ml-8 focus:outline-none">
        </div>
            <div class="m-4" v-if="searchResults.length > 0">
                <div v-for="i in searchResults" :key="i.user_id" class="flex items-center hover:bg-[#2D2D3A] p-2 rounded-xl cursor-default" @click="$router.push(`/user/${i.user_id}`)">
